      ...
      justify-between max-sm:flex-col-reverse rounded-lg'>
        <form className='flex flex-col gap-5 p-10 flex-1'>
          ...
          <label htmlFor="avatar" className='flex items-center gap-3 cursor-pointer'> 
            ...
            <img src={selectedImage ? URL.createObjectURL(selectedImage):assets.avatar} alt='' className={`w-12 h-12
          ...
          placeholder='Write a Profile bio' required className='p-2 border border-gray-500 rounded-md 
          focus:ring-2 focus:ring-violet-500' rows={4} ></textarea>

          <button type='submit' className='bg-gradient-to-r from-purple-400 to-violet-600 text-white p-2 rounded-full text-lg cursor-pointer'>Save</button>

        </form>
        <img src={authUser?.profilePic || assets.logo_icon} alt="" className={`${selectedImage ? 'rounded-full' : ''} max-w-44 aspect-square rounded-full mx-10 max-sm:mt-10`}/>
      </div>
    </div>
  )
}
