...
const SideBar = () => {
  ...
  const { users, selectedUsers, getUsers, sendMessage, setSelectedUsers, unseenMessages, setUnseenMessages } = useContext(ChatContext)
    ...
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white
      ...
      <div className='flex flex-col '>
          {filteredUsers.map((user,index) => (
            <div
            onClick={() => {setSelectedUsers(user) , setUnseenMessages(prev => ({...prev, [user._id]:0})}} 
            ...
            ${selectedUsers?._id === user._id && 'bg-[#282142]/50'}`}>
              <img src={user?.profilePic || assets.avatar_icon} alt='user profile'
                    className='w-[35px] aspect-ratio-1 rounded-full' />
